...
const USER_GROWTH_DATA = [
  { month: 'Jan', farmers: 45, buyers: 80, total: 125 },
  { month: 'Feb', farmers: 65, buyers: 120, total: 185 },
  { month: 'Mar', farmers: 90, buyers: 170, total: 260 },
  { month: 'Apr', farmers: 120, buyers: 230, total: 350 },
  { month: 'May', farmers: 160, buyers: 310, total: 470 },
  { month: 'Jun', farmers: 210, buyers: 420, total: 630 },
  { month: 'Jul', farmers: 270, buyers: 540, total: 810 },
];

const REVENUE_BAR_DATA = [
  { category: 'Crops', revenue: 18500 },
  { category: 'Vegetables', revenue: 12400 },
  { category: 'Machinery', revenue: 45000 },
  { category: 'Fertilizers', revenue: 8900 },
  { category: 'Seeds', revenue: 6700 },
];
...
const AdminDashboardHomePage = () => {
  ...
  const STATUS_PIE_DATA = [
    { name: 'Active / Approved', value: activeProducts || 12, color: '#16503b' },
    { name: 'Pending Review', value: pendingProducts || 4, color: '#f59e0b' },
    { name: 'Rejected', value: products.filter((p: any) => p.status === 'rejected').length || 2, color: '#ef4444' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-gray-900 via-slate-800 to-emerald-950 p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-300 text-sm mt-1">
            Monitor real-time user registrations, listing approvals, and platform analytics.
          </p>
        </div>
        <div className="flex items-center gap-2 bg-white/10 px-4 py-2.5 rounded-2xl backdrop-blur-md">
          <TrendingUp className="text-emerald-400" size={20} />
          <span className="text-xs font-bold">System Status: All Services Operational</span>
        </div>
      </div>
            ...
            >
              <div>
                ...
                <h3 className="text-3xl font-black text-gray-900 dark:text-white mt-1">
                  {card.loading ? <Loader2 className="animate-spin" size={24} /> : card.value}
                </h3>
              </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
                <LineChartIcon className="text-[#16503b]" size={20} /> User Acquisition & Growth
              </h2>
              <p className="text-xs font-semibold text-gray-400">Monthly new registered farmers & buyers</p>
            </div>
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={USER_GROWTH_DATA} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                />
                <Legend />
                <Area type="monotone" dataKey="farmers" stroke="#16503b" fillOpacity={1} fill="url(#colorFarmers)" name="Farmers" />
                <Area type="monotone" dataKey="buyers" stroke="#3b82f6" fillOpacity={1} fill="url(#colorBuyers)" name="Buyers" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0b1120] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col justify-between">
          <div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2 mb-1">
              <PieChartIcon className="text-[#16503b]" size={20} /> Listing Status Breakdown
            </h2>
            <p className="text-xs font-semibold text-gray-400 mb-4">Approval status of all marketplace items</p>
          ...
          <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-gray-800">
            {STATUS_PIE_DATA.map(item => (
              <div key={item.name} className="flex items-center justify-between text-xs font-bold">
                ...
                <span className="text-gray-900 dark:text-white">{item.value} items</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#0b1120] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
              <BarChart3 className="text-[#16503b]" size={20} /> Total Volume by Category
            </h2>
            <p className="text-xs font-semibold text-gray-400">Total traded value per category across AgroVision AI</p>
          </div>
        </div>

        <div className="h-[280px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={REVENUE_BAR_DATA} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
              ...
              />
              <Legend />
              <Bar dataKey="revenue" fill="#16503b" radius={[8, 8, 0, 0]} name="Total Volume ($)" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
